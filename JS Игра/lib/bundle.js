...
/******/ ([
	...
	/***/ function(module, exports, __webpack_require__) {
		...
		document.addEventListener('DOMContentLoaded', () => {
			...
			const imageNames = ['isit', 'Max', 'Nekit', 'Sava', 'Cisco','BossEvil', 'Boss', 'supplyRad', 'supplyPower', 'supplyHP', 'supplyRate', 'supplySpeed', 'playerBullet', 'enemyBullet', 'ship',
			...
			let audio = {};
			const audioNames = ['boss','bossBullet', 'TMR', 'main', 'shot', 'explosion'];
...
/***/ function(module, exports, __webpack_require__) {
	...
	const defaultLives = 5;
		...
		constructor(images, audio, height) {
			...
			this.pacani = false;
			this.theend = false;
			if (typeof this.audio.main.loop == 'boolean')
			{
				this.audio.main.loop = true;
			}
			else
			{
				this.audio.main.addEventListener('ended', function() {
					this.audio.main.currentTime = 0;
					this.audio.main.play();
				}, false);
			}
			this.audio.main.volume = 0.5;
			this.audio.bossBullet.volume = 0.1;
			this.audio.main.play();
		}
		...
		addEnemies() {
			if (!this.isOver) {
				for(let i = 0; i < this.NUM_ENEMIES; i++) {
					let enemy = new Enemy({
						game: this,
						health: this.ship.fireRate*this.level + this.level*this.powerUpGet,
						vel: [0, 0],
						radius: 40,
						speed: 40
					});
	...
	nextLevel() {
		...
		this.NUM_ENEMIES = 5 + Math.floor(this.level/5);
	}
}
...
secondAchieve(){
	if(!this.pacani){
			...
			if (typeof this.audio.boss.loop == 'boolean')
			{
				this.audio.boss.loop = true;
			}
			else
			{
				this.audio.boss.addEventListener('ended', function() {
					this.audio.boss.currentTime = 0;
					this.audio.boss.play();
				}, false);
			}
			this.audio.boss.volume = 0.5;
			this.audio.bossBullet.volume = 0.1;
			this.audio.boss.play();
		...
		this.pacani = 1;
		    ...
		    // hold enemies while appearance is drawing
		    	this.enemies.forEach(enemy => {
		    		enemy.vel = Utils.randomVector(4);
		    	});
		    }, 3000);
	}
}
...
plsKillHim(){
	...
	bossFight(){
	this.renderingEnemies = false;
	this.bossHealth *= this.ship.fireRate*this.ship.power;
	this.destroyAll();
	this.boss = 1;
	setTimeout(() => {
		let enemy = new Enemy({
...
spawnmobs(){
	if(this.boss && this.enemies.length == 1 && !this.theend && !this.pacani){
			if(this.enemies[0].health < this.bossHealth - this.adds*this.bossHealth/5){
		this.enemies[0].evade();
		this.enemies[0].appear();
		this.adds++;
		for(let i = 0; i < 10; i++) {
			let enemy = new Enemy({
				game: this,
				health: this.ship.fireRate*this.level + this.level*this.powerUpGet*3,
				vel: [0, 0],
				radius: 40,
				speed: 20
			});
...
/*bossFightLevel2(){
	this.enemies[0].explode();
	this.enemies[0].remove();
	this.boss = 2;
		let enemy = new Enemy({
						game: this,
						health: this.ship.fireRate*this.ship.power*this.bossHealth/2,
						vel: [0, 0],
						radius: this.DIM_Y/3.2,
						boss: 2,
						speed: 0
					});
					this.startShot();
...
destroyAll(){
	if(!this.boss){
		...
		this.ship.destroyAllBullet();
		setTimeout(() => {
		var n = this.enemies.length
		for (var i = n - 1; i >= 0; i--) {
			this.enemies[i].decreaseHealth(this.enemies[i].health);
		}}, 1000);
		/*this.enemies.forEach(obj => {
			obj.decreaseHealth(this.level);
			console.log("KILLED");
		});*/
		this.power = 0;
	}
...
step() {
	...
	this.nextLevel();
	this.spawnmobs();
	if(this.enemies.length==0 && this.theend && this.isFinallyOver)
		this.gameOver();
}
...
draw(ctx) {
	...
	if (this.isOver && !this.theend) {
		this.begin ? this.drawIntro(ctx) : this.drawGameOver(ctx);
	} else if (this.isOver && this.theend && !this.lives) {
		this.drawEndGame(ctx);
	} else if(this.isOver && this.theend) {
		this.drawGameOver(ctx);
	} else {
		this.drawScore(ctx);
		this.drawLevel(ctx);
		this.drawLives(ctx);
		if(!this.boss) this.drawPower(ctx);
	}
...
drawIntro(ctx) {
	ctx.fillStyle = 'red';
	ctx.font = 'bold 84px VT323';
	ctx.fillText(`SPACE MAX`, 150, 150);
	ctx.fillStyle = 'red';
	ctx.font = 'bold 70px VT323';
	ctx.fillText(`HIT ENTER TO FLEX`, 70, 300);
	...
	ctx.fillText(`SHOOT`, 140, 600);

	ctx.fillStyle = 'yellow';
	ctx.font = 'bold 50px VT323';
	ctx.fillText(`MOVE`, 470, 600);
}
...
drawGameOver(ctx) {
	...
	ctx.fillText(`YOUR SCORE`, 195, 300);
	...
	ctx.fillStyle = 'red';
	ctx.font = 'bold 70px VT323';
	ctx.fillText(`HIT ENTER TO REFLEX`, 40, 500);
}

drawEndGame(ctx) {
	ctx.fillStyle = 'red';
	ctx.font = 'bold 84px VT323';
	ctx.fillText(`CONGRATULATIONS?`, 50, 200);

	ctx.fillStyle = 'red';
	ctx.font = 'bold 56px VT323';
	ctx.fillText(`FIRST KEY FOUND`, 170, 300);

	ctx.fillStyle = 'red';
	ctx.font = 'bold 56px VT323';
	ctx.fillText(`FIND TWO MORE OR LOSE `, 110, 350);
}

drawPower(ctx) {
	ctx.fillStyle = 'red';
	ctx.font = 'bold 50px VT323';
	ctx.fillText(`POWER:${this.power}%`, 10, this.DIM_Y-30);
}

drawLevel(ctx) {
	ctx.fillStyle = 'red';
	ctx.font = 'bold 50px VT323';
	if(!this.boss) ctx.fillText(`LEVEL ${this.level}`, 230, 50);
	else ctx.fillText(`BOSS LEVEL`, 200, 50);
}

drawLives(ctx) {
	    ctx.fillStyle = 'red';
	    ctx.font = 'bold 35px VT323';
	    ctx.fillText(`LIVES:${this.lives}`, 480, 50);
	...
	increaseScore(rate) {
		this.score += Math.floor(rate);
		if(this.score >= 5000 && !this.first){
			this.firstAchieve();
		}
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	class Enemy extends MovingObject {
		constructor(options) {
			...
			if(!options.game.boss)
				options.pos = options.game.randomPosition(options.radius);
			else options.pos = [300,options.game.DIM_Y / 4];
			...
			this.immune = false;
			this.lastboss = options.lastboss;
			this.isit = options.isit;
			this.currFrame = 0;
		}
		...
		decreaseHealth(points) {
			this.health -= points;
			if (this.health <= 0) {
				this.game.increaseScore(80);
				this.explode();
				this.tossSuply();
				this.remove();
				if(this.boss == 1 && !this.pacani) {this.game.secondAchieve();
				if(this.isit){
					this.game.isFinallyOver = 1;
				}
				if(this.lastboss && !this.isit)
					if(this.game.enemies.length==0)
						this.game.plsKillHim();
		}
	}
}
		approxShipPos(scale) {
		...
		fireBullet() {
			if(!this.boss || this.lastboss){
				let shipApproxPos = this.approxShipPos(1 + 2/(this.game.level));
				...
				this.game.addBullet({
					...
					vel: vel,
					pos: this.pos,
					radius: DEFAULTS.bulradius + this.game.level,
					shootedBy: Enemy,
					power: 10,
					speed: 6
				});
			...
			let shipApproxPos;
				...
				shipApproxPos = this.approxShipPos(1);
				bulpos.push(Utils.sumVectors([[300,450],
					[-this.radius / 4, -this.radius + 200]]));
				bulpos.push(Utils.sumVectors([[300,450],
					[this.radius / 4, -this.radius + 200]]));

			if(this.game.theend){
				shipApproxPos = this.approxShipPos(5);
			}
			let vel = Utils.vectorBetween(this.pos, shipApproxPos);
				bulpos.forEach(pos => {
				this.game.addBullet({
		...
		tossSuply() {
			...
			if(rand <= 18 && !this.game.boss){
				let shipApproxPos = this.approxShipPos(1);
				let vel = Utils.vectorBetween(this.pos, shipApproxPos);
				this.game.addBullet({
					game: this.game,
					vel: vel,
			...
			if(rand <= 10 && !this.game.boss){
				...
				this.game.addBullet({
					...
					radius: 60,
					power: 10,
					shootedBy: 2, //fireRate
					speed: 5
				});
			}
			rand = Math.floor(Math.random()*100);
			if(rand <= 8 && !this.game.boss){
				let shipApproxPos = this.approxShipPos(1);
				let vel = Utils.vectorBetween(this.pos, shipApproxPos);
				this.game.addBullet({
					game: this.game,
		...
		draw(ctx, images) {
			if (this.immune && this.boss) ctx.globalAlpha = .5;
			...
			ctx.drawImage(
	     	...
	     	this.currFrame = ++this.currFrame%4;
	    	}
			else if(this.lastboss && !this.isit){
				let pic = images.Sava;
				switch (this.lastboss){
					case 1:
					pic = images.Max;
					break;
					case 2:
	...
	class Bullet extends MovingObject {
		...
		collideWith(other) {
			if (other instanceof Ship && this.shootedBy === Enemy && !other.protect) {
				...
				return true;
			} else if ( other instanceof Enemy && this.shootedBy === Ship && !other.immune) {
				...
				return true;
			} else if (other instanceof Ship && this.shootedBy === 0) {
				this.game.ship.powerUp();
				this.game.power += 8;
				if(this.game.power > 100)
					this.game.power = 100;
				this.remove();
				return true;
			} else if (other instanceof Ship && this.shootedBy === 1) {
				this.game.ship.healthUp();
				this.game.power += 8;
				if(this.game.power > 100)
					this.game.power = 100;
				this.remove();
				return true;
			} else if (other instanceof Ship && this.shootedBy === 2) {
				this.game.ship.fireRateUp();
				this.game.power += 8;
				if(this.game.power > 100)
					this.game.power = 100;
				this.remove();
				return true;
			} else if (other instanceof Ship && this.shootedBy === 3) {
				this.game.ship.radiusUp();
				this.game.power += 8;
				if(this.game.power > 100)
					this.game.power = 100;
				this.remove();
				return true;
			} else if (other instanceof Ship && this.shootedBy === 4) {
				this.game.ship.speedUp();
				this.game.power += 8;
				if(this.game.power > 100)
					this.game.power = 100;
				this.remove();
		...
		draw(ctx, images) {
			...
			if (this.shootedBy.name === 'Enemy') {
				...
				size = 20;
			} else if (this.shootedBy.name === 'Ship'){
				img = images.playerBullet;
				size = 192;
			} else if (this.shootedBy === 0){
				img = images.supplyPower;
				size = 150;
			} else if (this.shootedBy === 1){
				img = images.supplyHP;
				size = 150;
			} else if (this.shootedBy === 2){
				img = images.supplyRate;
				size = 150;
			} else if (this.shootedBy === 3){
				img = images.supplyRad;
				size = 150;
			} else if (this.shootedBy === 4){
				img = images.supplySpeed;
				size = 150;
			} else {
				img = images.Cisco;
				size = 207;
	      ...
	      if(this.shootedBy.name === 'Ship') this.currFrame = 5;
	      if(this.shootedBy === 10) {
	      	this.currFrame = ++this.currFrame%24;
	      }
	  }
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	const DEFAULTS = {
		...
		fps: 60,
	  	fireRate: 5,
	  	bulletRad: 15,
	  	firePower: 3 // bullets per second
	};
	...
	class Ship extends MovingObject {
		constructor(options) {
			...
			options.game.DIM_Y - DEFAULTS.radius - 50];
	    ...
	    this.power = DEFAULTS.firePower;
	    this.protect = false;
	    this.speed = 6;
	    this.interval;
	    this.bulletRad = DEFAULTS.bulletRad;
	...
	handleMoves(pressedKeys) {
		...
		pressedKeys.forEach(key => {
			if (Ship.MOVES[key]) {
	      	...
	      	if (this.frames > DEFAULTS.fps / this.fireRate) {
	      		this.fireBullet();
	      		this.frames = 0;
	      	} else {
	      		this.frames += 1;
	      	}
	      }	else if(key == 17 && this.game.power == 100 && this.game.enemies.length) {
	...
	powerUp() {
		this.power+= 0.3;
		if(this.power >= 3){
			this.game.powerUpGet = 1;
		}
	}

	healthUp() {
		if(this.game.lives < 10) this.game.lives++;
	}
	...
	fireBullet() {
		this.game.audio.shot.volume = 0.1;
		if (!this.game.audio.shot.muted) 	this.game.audio.shot.cloneNode().play();
		let bulpos = [];
		bulpos.push(Utils.sumVectors([this.pos,
			[0, 0]]))
		if(true){
			bulpos.push(Utils.sumVectors([this.pos,
	...
	destroyAllBullet() {
		let leftPos = Utils.sumVectors([[200,this.game.DIM_Y - 100],
			[-this.radius / 1.5, -this.radius]]);
		let rightPos = Utils.sumVectors([[400,this.game.DIM_Y - 100],
			[this.radius / 1.5, -this.radius]]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	class Star extends MovingObject {
		constructor(options) {
			options.radius = Math.random() + .5;
			options.speed = Math.pow(options.radius, .5);
